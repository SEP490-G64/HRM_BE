...
public class NotificationServiceImpl implements NotificationService {
    ...
    public Page<Notification> getByPaging(int pageNo, int pageSize, String sortBy) {
        ...
        return notificationRepository
                .findAll(pageable)
                .map(dao -> notificationMapper.toDTO(dao));
    }
    ...
    public Notification create(NotificationCreateRequest notification) {
        ...
        if (notification.getBranchBatchId() != null) {
            branchBatch = entityManager.getReference(BranchBatchEntity.class, notification.getBranchBatchId());
            if (branchBatch == null) {
                throw new HrmCommonException("Branch Batch not found with id: " + notification.getBranchBatchId());
            }
        } else {
            branchBatch = null;
        }
    ...
    public Notification update(NotificationUpdateRequest notification) {
        NotificationEntity oldNotificationDetailEntity = notificationRepository.findById(notification.getId()).orElse(null);
