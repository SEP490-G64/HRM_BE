...
public class InboundServiceImpl implements InboundService {
        ...
        inboundEntity
                ...
                .forEach(
                        inboundDetail -> {
                            ...
                            Integer totalQuantity = inboundBatchDetailRepository
                                    .findTotalQuantityByInboundAndProduct(inboundId, product);

                            Integer quantity =
                                    totalQuantity != 0 ? totalQuantity
                                            : (inboundDetail.getReceiveQuantity() != null
                                            ? inboundDetail.getReceiveQuantity() : 0); // Assume this represents the
    ...
    private void updateAverageInboundPricesForBatches(BatchEntity batch) {
        ...
        List<InboundBatchDetailEntity> allInboundBatchDetails = inboundBatchDetailRepository.findAllByBatchId(batch.getId());
        ...
        if (totalQuantity > 0) {
            BigDecimal averageProductPrice = totalPrice.divide(BigDecimal.valueOf(totalQuantity), 2, RoundingMode.HALF_UP);
    ...
    private void updateAverageInboundPricesForProductsAndInboundTotalPrice(InboundEntity inbound) {
        ...
        List<InboundDetailsEntity> allDetails = inboundDetailsRepository.findInboundDetailsWithCategoryByInboundId(inbound.getId());
        ...
        for (InboundDetailsEntity inboundDetails : allDetails) {
            ...
            List<BatchEntity> allBatches = batchRepository.findAllByProductId(inboundDetails.getProduct().getId());
            ...
            if (!allBatches.isEmpty()) {
                for (BatchEntity batch : allBatches) {
                    List<BranchBatchEntity> allBranchBatches = branchBatchRepository.findByBatchId(batch.getId());
                    ...
                    InboundBatchDetailEntity inboundBatchDetails = inboundBatchDetailRepository
                            .findByBatch_IdAndAndInbound_Id(batch.getId(), inbound.getId()).orElse(null);
                    if (inboundBatchDetails != null) {
                        BigDecimal originalPrice = inboundBatchDetails.getInboundPrice()
                        ...
                        if (discount != 0.0) {
                            inboundTotalPrice = inboundTotalPrice.subtract(originalPrice.multiply(BigDecimal.valueOf(discount)));
                        }
                    }
                }
            }
            else {
                totalPrice = inboundDetailsRepository.findTotalPriceForProduct(inboundDetails.getProduct().getId());
                totalQuantity = branchProductRepository.findTotalQuantityForProduct(inboundDetails.getProduct().getId());

                BigDecimal originalPrice = inboundDetails.getInboundPrice()
