...
public interface InboundRepository
  ...
  @Query("SELECT SUM(COALESCE(i.totalPrice, 0.0)) FROM InboundEntity i WHERE (:branchId IS NULL OR i.toBranch.id = :branchId)")
  ...
  @Query(value = "WITH DateRange AS (\n" +
          ...
          "InboundData AS (\n" +
          "    SELECT \n" +
          "        CAST(i.created_date AS DATE) AS period_start,  -- Chỉ dùng ngày mà không cần tính theo tuần/tháng/quý/năm\n" +
          "        SUM(i.total_price) AS inbound\n" +
          "    FROM inbound i\n" +
          "    WHERE CAST(i.created_date AS DATE) BETWEEN CAST(:startDate AS DATE) AND CAST(:endDate AS DATE)\n" +
          "    AND (:branchId IS NULL OR i.to_branch_id = :branchId)  -- Điều kiện chi nhánh\n" +
          "    GROUP BY CAST(i.created_date AS DATE)  -- Nhóm theo ngày\n" +
          "),\n" +
          "OutboundData AS (\n" +
          "    SELECT \n" +
          "        CAST(o.created_date AS DATE) AS period_start,  -- Chỉ dùng ngày mà không cần tính theo tuần/tháng/quý/năm\n" +
          "        SUM(o.total_price) AS outbound\n" +
          "    FROM outbound o\n" +
          "    WHERE CAST(o.created_date AS DATE) BETWEEN CAST(:startDate AS DATE) AND CAST(:endDate AS DATE)\n" +
          "    AND (:branchId IS NULL OR o.from_branch_id = :branchId)  -- Điều kiện chi nhánh\n" +
  ...
  List<Object[]> getInboundOutboundDataByDay(@Param("startDate") String startDate, @Param("endDate") String endDate,@Param("branchId") Long branchId);
  ...
  @Query(value = "WITH DateRange AS (\n" +
          ...
          "InboundData AS (\n" +
          "    SELECT \n" +
          "        DATE_TRUNC('week', CAST(i.created_date AS DATE)) AS period_start,  -- Trunc date theo tuần\n" +
          "        SUM(i.total_price) AS inbound\n" +
          "    FROM inbound i\n" +
          "    WHERE CAST(i.created_date AS DATE) BETWEEN CAST(:startDate AS DATE) AND CAST(:endDate AS DATE)\n" +
          "    AND (:branchId IS NULL OR i.to_branch_id = :branchId)  -- Kiểm tra điều kiện branchId\n" +
          "    GROUP BY \n" +
          "        DATE_TRUNC('week', CAST(i.created_date AS DATE))\n" +
          "),\n" +
          "OutboundData AS (\n" +
          "    SELECT \n" +
          "        DATE_TRUNC('week', CAST(o.created_date AS DATE)) AS period_start,  -- Trunc date theo tuần\n" +
          "        SUM(o.total_price) AS outbound\n" +
          "    FROM outbound o\n" +
          "    WHERE CAST(o.created_date AS DATE) BETWEEN CAST(:startDate AS DATE) AND CAST(:endDate AS DATE)\n" +
          "    AND (:branchId IS NULL OR o.from_branch_id = :branchId)  -- Kiểm tra điều kiện branchId\n" +
          "    GROUP BY \n" +
          ...
          "LEFT JOIN InboundData i ON dr.date = i.period_start\n" +
          "LEFT JOIN OutboundData o ON dr.date = o.period_start\n" +
          "ORDER BY dr.date ASC;", nativeQuery = true)
  List<Object[]> getInboundOutboundDataByWeek(@Param("startDate") String startDate, @Param("endDate") String endDate, @Param("branchId") Long branchId);
  ...
  @Query(value = "WITH DateRange AS (\n" +
          ...
          "LEFT JOIN OutboundData o ON dr.date = o.period_start\n" +
          "ORDER BY dr.date ASC;", nativeQuery = true)
  List<Object[]> getInboundOutboundDataByMonth(@Param("startDate") String startDate, @Param("endDate") String endDate,@Param("branchId") Long branchId);
  ...
  @Query(value = "WITH DateRange AS (\n" +
          ...
          "LEFT JOIN OutboundData o ON dr.date = o.period_start\n" +
          "ORDER BY dr.date ASC;", nativeQuery = true)
  List<Object[]> getInboundOutboundDataByQuarter(@Param("startDate") String startDate,
  ...
  @Query(value = "WITH DateRange AS (\n" +
          ...
          "LEFT JOIN OutboundData o ON dr.date = o.period_start\n" +
          "ORDER BY dr.date ASC;", nativeQuery = true)
  List<Object[]> getInboundOutboundDataByYear(@Param("startDate") String startDate, @Param("endDate") String endDate,@Param("branchId") Long branchId);
}
