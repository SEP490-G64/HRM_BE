...
public interface ProductRepository
  ...
  @Query(
      "SELECT DISTINCT p FROM ProductEntity p JOIN p.branchProducs bp LEFT JOIN FETCH p.batches b"
          ...
          + " :branchId AND bp.quantity > 0 AND (b IS NULL OR bb.quantity > 0) AND (b IS NOT NULL"
          + " AND b.expireDate >= CURRENT_TIMESTAMP) AND p.status != 'DA_XOA' AND p.sellPrice IS"
          + " NOT NULL")
  ...
  @Query(
          value =
                  "WITH OutboundProduct AS (\n"
                          ...
                          + "    COALESCE(ip.inbound_quantity, 0) + COALESCE(op.outbound_quantity_converted, 0) + COALESCE(obp.outbound_quantity_converted, 0) AS total_quantity,\n"
                          + "    COALESCE(ip.inbound_total, 0) AS inbound_transaction,\n"
                          + "    COALESCE(op.outbound_total, 0) + COALESCE(obp.outbound_total, 0) AS outbound_transaction,\n"
                          + "    COALESCE(ip.inbound_total, 0) + COALESCE(op.outbound_total, 0) + COALESCE(obp.outbound_total, 0) AS total_transaction,\n"
                          + "    uom.unit_name\n"
                          + "FROM product p\n"
