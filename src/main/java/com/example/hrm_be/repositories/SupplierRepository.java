...
public interface SupplierRepository extends JpaRepository<SupplierEntity, Long> {
  ...
  @Query(
          value = "WITH InboundTotal AS (\n" +
                  ...
                  "       s.supplier_name,\n" +
                  "       s.address,\n" +
                  "       COALESCE(it.inbound_quantity, 0) + COALESCE(ot.outbound_quantity, 0) AS quantity,\n" +
                  "       COALESCE(it.inbound_total, 0) + COALESCE(ot.outbound_total, 0) AS total_transaction\n" +
                  "FROM supplier s\n" +
