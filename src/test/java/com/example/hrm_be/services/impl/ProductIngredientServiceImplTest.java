...
class ProductIngredientServiceImplTest {
    ...
    private ProductIngredientEntity ingredientEntity; // Đối tượng ProductIngredientEntity dùng để test
    ...
    void shouldGetById() {
        ...
        verify(ingredientRepository, times(1)).findById(1L); // Đảm bảo rằng phương thức đã được gọi đúng 1 lần
    }
    ...
    void shouldGetByPaging() {
        ...
        assertEquals("Test Ingredient", result.getContent().get(0).getName()); // Kiểm tra tên nguyên liệu
        verify(ingredientRepository, times(1)).findAll(any(Pageable.class)); // Đảm bảo phương thức đã được gọi
    }
    ...
    void shouldCreateIngredient() {
        ...
        when(ingredientRepository.existsByName(anyString())).thenReturn(false); // Nguyên liệu không tồn tại
        when(ingredientRepository.save(any(ProductIngredientEntity.class))).thenReturn(ingredientEntity);
        when(ingredientMapper.toDTO(any(ProductIngredientEntity.class))).thenReturn(ingredientDTO);
        ...
        assertNotNull(result);
        assertEquals("Test Ingredient", result.getName());
        verify(ingredientRepository, times(1)).save(any(ProductIngredientEntity.class)); // Đảm bảo rằng phương thức save đã được gọi
    }
    ...
    void shouldThrowExceptionWhenCreatingExistingIngredient() {
        ...
        HrmCommonException exception = assertThrows(HrmCommonException.class, () -> productIngredientService.create(ingredientDTO));

        // Kiểm tra thông điệp ngoại lệ
        assertEquals(HrmConstant.ERROR.INGREDIENT.EXIST, exception.getMessage());
        verify(ingredientRepository, never()).save(any(ProductIngredientEntity.class)); // Đảm bảo rằng save không được gọi
    }
    ...
    void shouldUpdateIngredient() {
        ...
        assertNotNull(result);
        assertEquals("Test Ingredient", result.getName());
        verify(ingredientRepository, times(1)).save(any(ProductIngredientEntity.class)); // Đảm bảo phương thức save đã được gọi
    }
    ...
    void shouldThrowExceptionWhenUpdatingNonExistentIngredient() {
        ...
        HrmCommonException exception = assertThrows(HrmCommonException.class, () -> productIngredientService.update(ingredientDTO));
    ...
    void shouldDeleteIngredient() {
        ...
        verify(ingredientRepository, times(1)).deleteById(1L); // Đảm bảo phương thức deleteById đã được gọi
    }
    ...
    void shouldDeleteIngredientWithBlankId() {
        ...
        verify(ingredientRepository, never()).deleteById(anyLong()); // Đảm bảo rằng deleteById không được gọi
    }
}
