...
class SupplierServiceImplTest {
    ...
    void shouldGetById() {
        ...
        verify(supplierRepository, times(1)).findById(1L); // Đảm bảo rằng phương thức đã được gọi đúng 1 lần
    }
    ...
    void shouldGetByPaging() {
        ...
        assertEquals("Test Supplier", result.getContent().get(0).getSupplierName()); // Kiểm tra tên nhà cung cấp
        verify(supplierRepository, times(1)).findAll(any(Pageable.class)); // Đảm bảo phương thức đã được gọi
    }
    ...
    void shouldCreateSupplier() {
        ...
        when(supplierRepository.existsBySupplierNameAndAddress(anyString(), anyString())).thenReturn(false); // Nhà cung cấp không tồn tại
        ...
        assertNotNull(result);
        assertEquals("Test Supplier", result.getSupplierName());
        verify(supplierRepository, times(1)).save(any(SupplierEntity.class)); // Đảm bảo rằng phương thức save đã được gọi
    }
    ...
    void shouldThrowExceptionWhenCreatingExistingSupplier() {
        ...
        verify(supplierRepository, never()).save(any(SupplierEntity.class)); // Đảm bảo rằng save không được gọi
    }
    ...
    void shouldUpdateSupplier() {
        ...
        assertNotNull(result);
        assertEquals("Test Supplier", result.getSupplierName());
        verify(supplierRepository, times(1)).save(any(SupplierEntity.class)); // Đảm bảo phương thức save đã được gọi
    }
    ...
    void shouldDeleteSupplier() {
        ...
        verify(supplierRepository, times(1)).deleteById(1L); // Đảm bảo phương thức deleteById đã được gọi
    }
    ...
    void shouldDeleteSupplierWithBlankId() {
        ...
        verify(supplierRepository, never()).deleteById(anyLong()); // Đảm bảo rằng deleteById không được gọi
    }
}