...
public class OutboundServiceImplTest {
    ...
    @Mock
    private EntityManager entityManager; // Add the EntityManager mock
    ...
    void setUp() {
        ...
        updatedOutbound = Outbound.builder()
    ...
    public void UTCID01_testGetById_SuccessfulRetrieval() {
        ...
        when(outboundMapper.convertToDtoBasicInfo(any(OutboundEntity.class))).thenReturn(outboundDetailDTO);
        when(unitOfMeasurementMapper.toDTO(any(UnitOfMeasurementEntity.class))).thenReturn(new UnitOfMeasurement());
    ...
    public void UTCID02_testGetById_ThrowsException_WhenOutboundNotFound() {
        ...
        verify(outboundRepository, times(1)).findById(anyLong());
        verifyNoMoreInteractions(outboundRepository, outboundMapper, unitOfMeasurementMapper, productService);
    }
    ...
    public void UTCID04_testGetById_FilteredBatches_IncludeAllBatchesFalse() {
        ...
        outboundEntity.setOutboundType(OutboundType.BAN_HANG); // Set to a type that should not include all batches
        Outbound result = outboundServiceImpl.getById(1L);

        List<Batch> filteredBatches = productBaseDTO.getBatches().stream()
    ...
    public void UTCID05_testGetById_FilteredBatches_IncludeAllBatchesTrue() {
        ...
        outboundEntity.setOutboundType(OutboundType.TRA_HANG); // Set to a type that should include all batches
        Outbound result = outboundServiceImpl.getById(1L);
    ...
    public void UTCID06_testGetById_SetProductDTOBatchesToNull() {
        ...
        when(outboundMapper.convertToDtoBasicInfo(any(OutboundEntity.class))).thenReturn(outboundDetailDTO);
        when(unitOfMeasurementMapper.toDTO(any(UnitOfMeasurementEntity.class))).thenReturn(new UnitOfMeasurement());
        when(productService.getBranchProducts(anyLong(), anyLong())).thenReturn(null); // Simulating null productBaseDTO
    ...
    public void UTCID01_testGetByPaging_SuccessfulRetrieval() {
        ...
        Page<Outbound> result = outboundServiceImpl.getByPaging(pageNo, pageSize, sortBy, direction,
    ...
    public void UTCID04_testGetByPaging_WithoutOptionalParameters() {
        ...
        Page<Outbound> result = outboundServiceImpl.getByPaging(pageNo, pageSize, sortBy, direction,
                branchId, null, null, null, null, null);
    ...
    public void UTCID01_testGetSpecification_Predicates() {
        ...
        Specification<OutboundEntity> specification = outboundServiceImpl.getSpecification(
                branchId, keyword, startDate, endDate, status, type);
        ...
        when(criteriaBuilder.greaterThanOrEqualTo(any(Path.class), eq(startDate))).thenReturn(mockPredicate);
    ...
    public void UTCID03_testApprove_Accept() {
        ...
        OutboundEntity updatedOutboundEntity = oldOutboundEntity.toBuilder()
                .isApproved(true)
    ...
    public void testSaveOutbound_Success() {
        ...
        lenient().when(outboundRepository.save(any(OutboundEntity.class))).thenReturn(updatedOutboundEntity);
        ...
        lenient().when(branchProductService.getByBranchIdAndProductId(anyLong(), anyLong())).thenReturn(branchProduct);
        lenient().when(branchProductMapper.toEntity(any(BranchProduct.class))).thenReturn(branchProductEntity);
        ...
        lenient().when(branchBatchService.findQuantityByBatchIdAndBranchId(anyLong(), anyLong())).thenReturn(BigDecimal.TEN);
    ...
    public void testSaveOutbound_QuantityCheckForProduct() {
        ...
        request.getOutboundProductDetails().get(0).setBatch(null); // Set batch to null to test product details

        Outbound result = outboundServiceImpl.saveOutbound(request);

        assertNotNull(result);
        verify(branchProductService).getByBranchIdAndProductId(fromBranchdto.getId(), productEntity.getId());
    ...
    public void testSaveOutbound_QuantityCheckForBatch() {
        ...
        lenient().when(batchService.getById(anyLong())).thenReturn(batch);
        lenient().when(batchMapper.toEntity(any(Batch.class))).thenReturn(batchEntity);
        lenient().when(branchBatchService.findQuantityByBatchIdAndBranchId(anyLong(), anyLong())).thenReturn(BigDecimal.ONE);

        request.getOutboundProductDetails().get(0).setBatch(batch); // Ensure batch is set for batch handling

        HrmCommonException exception = assertThrows(HrmCommonException.class, () -> {
            outboundServiceImpl.saveOutbound(request);
        });

        assertEquals("Số lượng hiện tại trong kho của lô BATCH1 chỉ còn 1, vui lòng nhập số lượng nhỏ hơn.", exception.getMessage());
    }
    ...
    public void testSaveOutbound_UpdateExistingProductDetail() {
        ...
        lenient().when(productService.getById(anyLong())).thenReturn(product);
        lenient().when(productMapper.toEntity(any(Product.class))).thenReturn(productEntity);
        lenient().when(branchProductService.getByBranchIdAndProductId(eq(fromBranchdto.getId()), eq(productEntity.getId()))).thenReturn(branchProduct); // Correct stubbing to return BranchProductEntity
        lenient().when(branchProductMapper.toEntity(any(BranchProduct.class))).thenReturn(branchProductEntity);
        lenient().when(branchBatchService.findQuantityByBatchIdAndBranchId(anyLong(), anyLong())).thenReturn(BigDecimal.TEN);
        lenient().when(outboundProductDetailService.findByOutboundAndProduct(anyLong(), anyLong())).thenReturn(existingProductDetail);
        lenient().when(outboundProductDetailMapper.toEntity(any(OutboundProductDetail.class))).thenAnswer(i -> {
        ...
        // Verify
        assertNotNull(result);
        verify(outboundProductDetailService).findByOutboundAndProduct(updatedOutboundEntity.getId(), productEntity.getId());
        verify(outboundProductDetailService).deleteByOutboundId(request.getOutboundId());
        verify(outboundDetailService).deleteByOutboundId(request.getOutboundId());
        verify(outboundRepository).save(any(OutboundEntity.class));
        verify(outboundProductDetailService).saveAll(anyList());
    ...
    public void testSaveOutboundForSell() {
        ...
        lenient().when(productService.getById(anyLong())).thenReturn(product);
        lenient().when(productMapper.toEntity(any(Product.class))).thenReturn(productEntity);
        lenient().when(branchProductService.getByBranchIdAndProductId(fromBranchEntity.getId(), productEntity.getId())).thenReturn(branchProduct);
        lenient().when(unitConversionService.convertToUnit(anyLong(), anyLong(), any(BigDecimal.class), any(UnitOfMeasurement.class), eq(true))).thenReturn(BigDecimal.TEN);
        lenient().when(unitConversionService.convertToUnit(anyLong(), anyLong(), any(BigDecimal.class), any(UnitOfMeasurement.class), eq(false))).thenReturn(BigDecimal.TEN);
        lenient().when(branchBatchService.findQuantityByBatchIdAndBranchId(anyLong(), anyLong())).thenReturn(BigDecimal.TEN);
        lenient().when(branchBatchService.findByProductAndBranchForSell(anyLong(), anyLong())).thenReturn(new ArrayList<>());
        lenient().when(outboundProductDetailService.findByOutboundAndProduct(anyLong(), anyLong())).thenReturn(null);
        lenient().when(outboundProductDetailMapper.toEntity(any(OutboundProductDetail.class))).thenAnswer(i -> {
            OutboundProductDetail detail = i.getArgument(0);
            OutboundProductDetailEntity entity = new OutboundProductDetailEntity();
            entity.setOutboundQuantity(detail.getOutboundQuantity());
        ...
        lenient().when(unitOfMeasurementMapper.toEntity(any(UnitOfMeasurement.class))).thenReturn(new UnitOfMeasurementEntity());
        ...
        verify(outboundProductDetailService).deleteByOutboundId(request.getOutboundId());
        verify(outboundDetailService).deleteByOutboundId(request.getOutboundId());
        verify(outboundRepository, times(2)).save(any(OutboundEntity.class)); // Verify save is called twice
        verify(outboundProductDetailService, times(2)).saveAll(anyList()); // Verify saveAll is called twice
    }
    ...
    public void testSaveOutboundForSell_withProductBatch() {
        ...
        lenient().when(branchBatchService.findByProductAndBranchForSell(anyLong(), anyLong())).thenReturn(productBranchBatches);
        lenient().when(unitConversionService.convertToUnit(anyLong(), anyLong(), any(BigDecimal.class), any(UnitOfMeasurement.class), eq(true))).thenReturn(BigDecimal.TEN);
        lenient().when(unitConversionService.convertToUnit(anyLong(), anyLong(), any(BigDecimal.class), any(UnitOfMeasurement.class), eq(false))).thenReturn(BigDecimal.TEN);
        lenient().when(batchService.getById(anyLong())).thenReturn(batch);
    ...
    public void testSaveOutboundForSell_insufficientAvailableForSell() {
        ...
        assertEquals(
                "Số lượng hiện tại trong kho của sản phẩm " +
                        ...
                        " chỉ còn " + branchBatch.getQuantity() +
    ...
    public void testSaveOutboundForSell_insufficientBatchQuantity() {
        ...
        lenient().when(branchBatchService.findByProductAndBranchForSell(anyLong(), anyLong())).thenReturn(productBranchBatches);
        lenient().when(unitConversionService.convertToUnit(eq(1L), eq(1L), eq(BigDecimal.valueOf(5)), eq(targetUnit), eq(false))).thenReturn(BigDecimal.valueOf(5)); // Ensure correct arguments

        // Stubbing for converting other quantities
        lenient().when(unitConversionService.convertToUnit(eq(1L), eq(1L), eq(BigDecimal.TEN), eq(targetUnit), eq(true))).thenReturn(BigDecimal.TEN);
        lenient().when(batchService.getById(anyLong())).thenReturn(batch);
        ...
        lenient().when(unitConversionService.convertToUnit(eq(1L), eq(1L), eq(BigDecimal.TEN), eq(targetUnit), eq(true))).thenReturn(convertedQuantity);
    ...
    public void testSubmitOutboundToSystem_InsufficientProductQuantity() {
        ...
        OutboundEntity outboundEntity = createOutboundEntityWithInsufficientProductQuantity(productEntity);
        doReturn(Optional.of(outboundEntity)).when(outboundRepository).findById(1L);

        // Correctly mock the save method
        doAnswer(invocation -> invocation.getArgument(0)).when(outboundRepository).save(any(OutboundEntity.class));
    ...
    private OutboundEntity createOutboundEntityWithInsufficientProductQuantity(ProductEntity productEntity) {
        OutboundEntity outboundEntity = new OutboundEntity();
        outboundEntity.setId(1L);
        outboundEntity.setStatus(OutboundStatus.KIEM_HANG);
        ...
        lenient().when(branchProductService.getByBranchIdAndProductId(1L, 1L)).thenReturn(branchProduct);
    ...
    private OutboundEntity createSavedOutboundEntity(ProductEntity productEntity, Batch batch) {
        ...
        when(branchProductService.getByBranchIdAndProductId(anyLong(), anyLong())).thenReturn(branchProduct);
    ...
    public void testCreateInnitOutbound_Success() {
        ...
        try (MockedStatic<WplUtil> wplUtilMock = Mockito.mockStatic(WplUtil.class)) {
            wplUtilMock.when(() -> WplUtil.generateNoteCode(any(LocalDateTime.class), anyString())).thenReturn(outboundCode);
