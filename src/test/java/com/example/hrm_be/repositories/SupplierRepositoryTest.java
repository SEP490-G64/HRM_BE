...
class SupplierRepositoryTest {
    ...
    void shouldCheckIfSupplierExistsByNameAndAddress() {
        ...
        when(supplierRepository.existsBySupplierNameAndAddress("Tech Supplies", "123 Tech Street")).thenReturn(true);

        // Gọi phương thức existsBySupplierNameAndAddress
        boolean exists = supplierRepository.existsBySupplierNameAndAddress("Tech Supplies", "123 Tech Street");

        // Kiểm tra rằng kết quả trả về là true
        assertTrue(exists);

        // Kiểm tra rằng phương thức existsBySupplierNameAndAddress đã được gọi một lần với tên và địa chỉ đúng
        verify(supplierRepository, times(1)).existsBySupplierNameAndAddress("Tech Supplies", "123 Tech Street");
    }
    ...
    void shouldReturnFalseWhenSupplierNotExistByNameAndAddress() {
        ...
        when(supplierRepository.existsBySupplierNameAndAddress("Nonexistent Supplier", "Unknown Address")).thenReturn(false);

        // Gọi phương thức existsBySupplierNameAndAddress
        boolean exists = supplierRepository.existsBySupplierNameAndAddress("Nonexistent Supplier", "Unknown Address");

        // Kiểm tra rằng kết quả trả về là false
        assertFalse(exists);

        // Kiểm tra rằng phương thức existsBySupplierNameAndAddress đã được gọi một lần với tên và địa chỉ không tồn tại
        verify(supplierRepository, times(1)).existsBySupplierNameAndAddress("Nonexistent Supplier", "Unknown Address");
    }
}
